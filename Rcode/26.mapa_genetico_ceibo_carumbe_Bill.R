...
#Que haces?
#Como te dijo LUis estuve revisando y curando los datos del Illumina 50K
# para Ceibo Carumbe. Te adjunto una planilla con los datos revisados 
#(saque los monomorficos, los extraños, etc etc). Los datos ya prontos serian losde la hoja "Ill50K Mapeo". Los marcadores que tienen la marca amarilla o
# celeste son los que pueden ser mas dudosos y que en caso de cosegregacion
## o lio serian los primeros en caer. Los verdes son los que requirieron
# menos revisacion.
# El orden en que estan es el de secuencia y en teoria es el correcto. 
# En la mayor parte parece ser asi, pero hay algunos que parecen estar en otro
# lado. 

# Hay muchos que pintan como cosegregantes (en parte tambien por el tamaño de la poblacion). 
# Calculo que habria que correr el mapa y despues eliminar 
# los cosegregantes (pero manteniendo el dato porque puede
                            #servirnos en otros trabajos).

# La ubicacion por cromosoma es la que viene de Illumina, pero debe ser validada, como siempre.

# Los marcadores del mapa que tenemos hoy, con el que han trabajado Pia,
# Nico y Andres, estan en la hoja "Eureka + SSR". 
# Deben, obviamente, ponerse en el analisis.
 # Bueno, avisame que planes tenes. Y confirmame que la direccion esta bien
 # Saludos
# Ariel
...
Ill50K_Mapeo.qtl.b  <- subset (Ill50K_Mapeo.qtl.a , 
                               ind=(ntyped(Ill50K_Mapeo.qtl.a) 
                                    >((totmar(Ill50K_Mapeo.qtl.a) * 80)/100)))
...
profileGen(Ill50K_Mapeo.qtl.d,id = "id", bychr =  TRUE, 
           stat.type = c("xo","dxo"),
...
summary(Ill50K_Mapeo.qtl.h)

plotMap(Ill50K_Mapeo.qtl.h)

write.cross (Ill50K_Mapeo.qtl.h, format= "tidy",
            filestem="./Data/procdata/Ill50K_Mapeo.qtl.h")



#geno.H <- pull.geno(Ill50K_Mapeo.qtl.h)
#geno.H [1:10, 1:10]
#colnames (geno.H)

#geno.H <- as.data.frame (geno.H) 

#geno.H <- geno.H %>%
 #         dplyr::mutate(id=rownames(geno.H)) %>%
...
GPxM <- read.table (file="./Data/rawdata/GPxM_Bill_Thomas.csv",
              header = TRUE, 
              sep = ",",
              quote = "\"'", 
              dec = ".",
...
mrk.eurk <- map.1.dt %>%
            dplyr::filter (str_detect (mrk_names , "M_")) %>%
            dplyr::rename (marker_2 = mrk_names)
  
mrk.eurk.1 <- mrk.eurk %>%
              inner_join (Ids_mkrs_eurekaSSR, by= "marker_2")

mrk.eurk.2 <- unique (mrk.eurk.1$cod_1)

str(GPxM)
str (map.1.dt)

# aca filtro en la matriz de bill lo que tenenos en la mtriz nuestra
GPxM_1 <- GPxM %>%
...
GPxM_2 <- GPxM %>%
          dplyr::filter (snp %in% mrk.eurk.2) %>%
          dplyr::arrange (genetic_map_chr) %>%
          dplyr::mutate ( cod_1 = snp) 

GPxM_2 <- GPxM_2 %>%
          inner_join(mrk.eurk.1 , by="cod_1")


mrk50k <-GPxM_1$snp
mrkeurk <-GPxM_2$marker_2

mrk.bill <- c(mrk50k, mrkeurk)
...
# Esos marcadores los vamos a usar para para filtrar
# de los grupos de ligamientos generados


geno.H <- read_delim (file="./Data/procdata/Ill50K_Mapeo.qtl.h_gen.csv", 
                            delim = ",", na = "-")
head(geno.H)
geno.H.1 <- geno.H %>%
            dplyr::filter (X1 %in% mrk.bill)

dim(geno.H.1)

### generar el geno de bill

write_delim (geno.H.1, path ="./Data/procdata/geno_bill.csv",
             delim = ",", na = "-")
...
GPxM_1.1 <- GPxM_1 %>%
            dplyr::select (c(snp, genetic_map_chr,cM))

head(GPxM_2)

GPxM_2.1 <- GPxM_2 %>%
            dplyr::select (c(marker_2, genetic_map_chr,cM)) %>%
            dplyr::rename (snp =marker_2 )

pmap_bill <- rbind (GPxM_1.1, GPxM_2.1)
  
pmap_bill <- pmap_bill %>%
            dplyr::arrange ( genetic_map_chr, cM)


write_delim (pmap_bill, path ="./Data/procdata/pmap_bill.csv",
             delim = ",", na = "-")


### generar la pheno
colnames (geno.H.1 )
id <- colnames (geno.H.1) [2:73]
random <- rnorm(72, 0, 1)
length(random)
length(id)

pheno_bill <- rbind(id, random)
pheno_bill <- as.data.frame (pheno_bill)
colnames (pheno_bill) <- id
pheno_bill  <- pheno_bill [-1,]

pheno_bill <- pheno_bill %>%
  dplyr::mutate (X="trait")%>%
  dplyr::select (X,everything())


write_delim (pheno_bill , path ="./Data/procdata/pheno_bill.csv",
             delim = ",", na = "-")

######### generamos el cross de bill #######
 
bill_Mapeo.qtl <- read.cross (format="tidy",
                                dir="./Data/procdata",
                                genfile ="geno_bill.csv", 
                                mapfile ="pmap_bill.csv" , 
                                phefile ="pheno_bill.csv" , 
                                na.strings="-",
                                genotypes=c("AA","BB"),
                                alleles=c("A","B"),
                                estimate.map=FALSE, 
...
bill_Mapeo.qtl <- jittermap(bill_Mapeo.qtl )
summary (bill_Mapeo.qtl )

plotMap (bill_Mapeo.qtl)


newmap <- est.map (bill_Mapeo.qtl)
...
## generamos los grupos de ligamiento nuevamente

bill_Mapeo.qtl.1 <- mstmap (bill_Mapeo.qtl, 
                              id="id",
                              bychr = FALSE, 
                              trace = TRUE, 
                              dist.fun ="kosambi")
...
plotMap (bill_Mapeo.qtl.1)

summary (bill_Mapeo.qtl.1)
nmar (bill_Mapeo.qtl.1)

bill_Mapeo.qtl.2 <- subset( bill_Mapeo.qtl.1, chr= c("L.10", "L.12", 
                                               "L.13", "L.14", "L.15","L.16",
                                                "L.17","L.18","L.19","L.2","L.20", 
                                                 "L.21","L.22", "L.26", "L.28", "L.3", 
                                                  "L.30", "L.4", "L.6", "L.7", "L.8", "L.9"))

plotMap (bill_Mapeo.qtl.2)
summary (bill_Mapeo.qtl.2)
nmar (bill_Mapeo.qtl.2)
summary (bill_Mapeo.qtl.1)

bill_Mapeo.qtl.3 <- mstmap (bill_Mapeo.qtl.2, 
                            id="id",
                            bychr = FALSE, 
                            trace = TRUE, 
                            dist.fun ="kosambi")

plotMap (bill_Mapeo.qtl.3)

summary (bill_Mapeo.qtl.3)
nmar (bill_Mapeo.qtl.3)

bill_Mapeo.qtl.4 <- mstmap (bill_Mapeo.qtl.3, 
                            id="id",
                            bychr = TRUE, 
                            trace = TRUE, 
                            dist.fun ="kosambi")

plotMap (bill_Mapeo.qtl.4)

write.cross (bill_Mapeo.qtl.4, format="tidy",
             filestem="./Data/procdata/bill_Mapeo.qtl.4")

######### armar con el pheno ####

pheno_nico <- read_delim (file="./Data/rawdata/pheno_nico.txt", 
                      delim = "\t", na = "NA")

geno.bill.4 <- read_delim (file="./Data/procdata/bill_Mapeo.qtl.4_gen.csv", 
                          delim = ",", na = "NA")

geno.bill.4 <- geno.bill.4 %>%
               dplyr::select (-c("IND3","IND99"))

id.bill <- colnames (geno.bill.4) [2:74]
dim(pheno_nico)
length(id.bill)

pheno_nico.1  <- pheno_nico %>%
                 dplyr::filter (genotypes %in% id.bill)

setdiff(id.bill, pheno_nico.1$genotypes)
setdiff(pheno_nico.1$genotypes, id.bill)

write_delim (geno.bill.4 , path ="./Data/procdata/geno.bill.4.csv",
             delim = ",", na = "-")


write_delim (pheno_nico.1 , path ="./Data/procdata/pheno_nico.1.csv",
             delim = ",", na = "-")

### 
geno.bill <- read_delim (file="./Data/procdata/geno_bill.csv", 
                           delim = ",", na = "-")

geno.bill.a <- geno.bill %>%
               dplyr::select (-c("IND3","IND99"))%>%
               dplyr::rename ("IND37"="IND37_IND57")


xx <- colnames (geno.bill.a) [2:74]
setdiff(xx, pheno_nico.1$genotypes)
setdiff(pheno_nico.1$genotypes, xx)

write_delim (geno.bill.a , path ="./Data/procdata/geno.bill.a.csv",
             delim = ",", na = "-")
