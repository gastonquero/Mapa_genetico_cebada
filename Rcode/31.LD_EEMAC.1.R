...
EEMAC.cross.jitter <- jittermap (EEMAC.cross, amount=1e-6)

map.x <- pull.map (EEMAC.cross.jitter, chr=1 , as.table=TRUE)


write.cross (EEMAC.cross.jitter, "csv", "./Data/rawdata/EEMAC.cross.jitter")


EEMAC.cross.tibble <- read_delim (file="./Data/rawdata/EEMAC.cross.jitter.csv",
                                 delim=",", quote = "\"", 
                                 na = "-", col_names = TRUE)

summary (EEMAC.cross)
plotMap (EEMAC.cross.jitter)

data.cross = EEMAC.cross.jitter
heterozygotes = FALSE
data.tibble = EEMAC.cross.tibble 
...
map.1 <- pull.map (EEMAC.cross, as.table=TRUE)

list.chr <- unique (map.1$chr) 


LD.cross <- run_LD (cross.data= EEMAC.cross , heterozygotes = FALSE, 
...
plot ( R2 ~ delta.bp, data=chr.3)

plot ( R2 ~ delta.bp, 
       xlim=c(0,10),data=chr.3)

plot ( R2 ~ delta.bp, data=chr.4)

plot ( R2 ~ delta.bp, data=chr.5)

plot ( R2 ~ delta.bp, data=chr.6)

plot ( R2 ~ delta.bp, data=chr.7)
...
dt.all.chrom <- lapply (index.chrom, function (filtro) {run_LD_decay (data = LD.cross , chrom= filtro, ini=1,l1= 0.25, l2=0.5, l3=0.75)
  
})


EEMAC.cross.chr3 <- subset(EEMAC.cross,  chr="3")

summary(EEMAC.cross.chr3 )

crossobj =EEMAC.cross.chr3
heterozygotes = FALSE
chr= 3
...
LD.EEMAC.cross.chr3.matrix <- plot.hm$LDmatrix

write.table (LD.EEMAC.cross.chr3.matrix , 
             file = "./Data/procdata/LD.EEMAC.cross.chr3.matrix.txt",
...
#########'
#########'
EEMAC.cross.tibble <- read_delim (file="./Data/rawdata/INNOVAGRO/cross_EEMAC.1.csv",
                                      delim=",", quote = "\"", 
                                      na = "-", col_names = TRUE)



head(EEMAC.cross.tibble)

EEMAC.cross.tibble <- EEMAC.cross.tibble %>%
  dplyr::select(-S)


EEMAC.cross.chr1.tibble_snp  <- EEMAC.cross.chr1.tibble %>%
  dplyr::select (starts_with("S"))

head (EEMAC.cross.chr1.tibble_snp)

EEMAC.cross.chr1.tibble_snp.1 <- EEMAC.cross.chr1.tibble_snp [-c(1,2),]


EEMAC.cross.chr1.tibble_bp <- EEMAC.cross.chr1.tibble_snp [2,]

mkr.chr1 <- colnames (EEMAC.cross.chr1.tibble_snp)

x.mk <- data.frame (mrks = mkr.chr1)

pos.chr1    <- EEMAC.cross.chr1.tibble_snp [2,]
pos.chr1.1  <- data.frame (pos =pos.chr1[1,])

pos.chr1.2  <- as.numeric(t(pos.chr1.1))

EEMAC.cross.chr1.tibble_bp.1 <- x.mk %>%
  dplyr::mutate (pos=pos.chr1.2)

### 
dt <- EEMAC.cross.chr1.tibble_bp.1 
...
write.cross (EEMAC.cross , format="csv",
            filestem="./Data/rawdata/chr.3.LD", chr=3, digits=NULL)


EEMAC.chr3.cross <- read.cross (format="csv",
...
chr.3.LD <- pull.map(EEMAC.cross, chr=1, as.table=FALSE)

JHI-Hv50k-2016-149280
JHI-Hv50k-2016-149900
