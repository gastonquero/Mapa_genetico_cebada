...
colnames (geno_eurekaSSR)

#### saco los parentales ###########
geno_eurekaSSR <- geno_eurekaSSR %>%
                  dplyr::select (-c(INDCA,INDCA1, INDCE,INDCE1))
...
id.eurekaSSR <- colnames (pre.one.eurekaSSR.2)[-c(1,2)]
...
### cargo los datos de Illumina

pre.one.I50K <- read_delim (file="./Data/rawdata/Ill50K.txt", 
                      delim = "\t", na = "NA")


colnames (pre.one.I50K)

#### saco las columnas que no van  ###########
pre.one.I50K  <- pre.one.I50K %>%
                 dplyr::select (-c(...7,...16,...29,...37,...42,...76,...80 ))

colnames (pre.one.I50K)

# reemplazo los alelos
pre.one.I50K [pre.one.I50K =="A"] <- "a" 
pre.one.I50K [pre.one.I50K =="B"] <- "b" 

# estos son las etiquetas de los marcadores        
pre.one.I50K  <- pre.one.I50K %>%
                 dplyr::mutate (Name = str_c ("*M", Name)) %>%
                 dplyr::mutate (sgre = "A.B")

## saco los individuos que estan en EurekaSRR
id.eurekaSSR 

id.I50K <- Ids_individuos_ceibo_carumbe %>%
           dplyr::filter (IND %in%id.eurekaSSR) %>%
           na.omit()

genos.I50K <- pre.one.I50K  %>%
              dplyr::select (-c (Index, Name, Chr,Position, sgre))
...
mrk.I50K <- pre.one.I50K %>%
            dplyr::select (c(Name, sgre))

pre.one.geno.I50k.2 <- bind_cols (mrk.I50K,geno.I50k.1)

head (pre.one.geno.I50k.2)

write_delim (pre.one.geno.I50k.2, file ="./Data/procdata/pre.one.geno.I50k.2.txt",
             delim = " ", na = "-")

#armo los metados 
info.cross.I50K <- "data type ri self" 

n.genotypes.I50K <- length (id.I50K$IND)
n.mrk.I50K <- nrow (mrk.I50K)

info.cross.num.I50K <- c(n.genotypes.I50K,n.mrk.I50K ,0,0,0)


##### armo la eureka 3 con los mismo individuos que la ceibo carumbe ####

pre.one.eurekaSSR.1a <- pre.one.eurekaSSR.1 %>%
                        dplyr::select (all_of ( id.I50K$IND))


# reemplazo los alelos
pre.one.eurekaSSR.1a [pre.one.eurekaSSR.1a =="A"] <- "a" 
pre.one.eurekaSSR.1a [pre.one.eurekaSSR.1a =="B"] <- "b" 
...
#uno las matrices
pre.one.eurekaSSR.3 <- bind_cols (marks, pre.one.eurekaSSR.1a)


id.eurekaSSR.3 <- colnames (pre.one.eurekaSSR.3)[-c(1,2)]

#exporto la matriz

write_delim (pre.one.eurekaSSR.3, file ="./Data/procdata/pre.one.eurekaSSR.3.txt",
             delim = " ", na = "-")
...
n.genotypes.eurekaSSR.3 <- length (id.eurekaSSR.3)
n.mrk.eurekaSSR <- nrow (marks)

info.cross.num.eurekaSSR <- c(n.genotypes.eurekaSSR.3,n.mrk.eurekaSSR,0,0,0)
...
onemap_eurekaSSR_riself.3 <- read_onemap (dir="./Data/procdata", 
                                        inputfile = "pre.one.eurekaSSR.3.txt" )


onemap_I50K_riself <- read_onemap (dir="./Data/procdata", 
                                          inputfile = "pre.one.geno.I50k.2.txt" )
...
plot (onemap_eurekaSSR_riself.3)
plot (onemap_I50K_riself)
...
(bins_eurekaSSR.3 <- find_bins (onemap_eurekaSSR_riself.3, exact = FALSE))

(bins_I50k <- find_bins (onemap_I50K_riself, exact = FALSE))
...
(onemap_eurekaSSR_riself.3_bins <- create_data_bins (onemap_eurekaSSR_riself.3, bins_eurekaSSR.3))

(onemap_I50K_riself_bins <- create_data_bins (onemap_I50K_riself, bins_I50k ))
...
#### combino las dos matrices 
I50K_eurekaSSR <- combine_onemap (onemap_I50K_riself_bins , onemap_eurekaSSR_riself.3_bins)
I50K_eurekaSSR

write_onemap_raw (I50K_eurekaSSR, file.name = "./Data/procdata/I50K_eurekaSSR.raw")
...
I50K_eurekaSSR_test <- test_segregation (I50K_eurekaSSR)



class (eurekaSSR_test)
class (I50K_eurekaSSR_test)

print (eurekaSSR_test)
print (I50K_eurekaSSR_test)

Bonferroni_alpha (eurekaSSR_test)

Bonferroni_alpha (I50K_eurekaSSR_test)


plot (eurekaSSR_test)
plot (I50K_eurekaSSR_test)
...
(dist.1 <- select_segreg (I50K_eurekaSSR_test, distorted = TRUE, numbers = FALSE)) #to show the markers numbers with segregation distortion
...
(no_dist.1 <- select_segreg (I50K_eurekaSSR_test, distorted = FALSE, numbers = TRUE)) #to show the markers numbers without segregation distortion
...
(LOD_sug.1 <- suggest_lod (I50K_eurekaSSR))
...
twopts_I50K_eurekaSSR <- rf_2pts (input.obj = I50K_eurekaSSR, LOD = LOD_sug.1)

class (twopts_eurekaSSR)

class (twopts_I50K_eurekaSSR)
...
mark_no_dist_riself.1 <- make_seq (twopts_I50K_eurekaSSR , no_dist.1)
class (mark_no_dist_riself.1)


#Forming the groups
(LGs_eurekaSSR <- group (mark_no_dist_riself,  LOD = LOD_sug, max.rf = 0.5))

(LGs_I50K_eurekaSSR <- group (mark_no_dist_riself.1,  LOD = LOD_sug.1, max.rf = 0.5))
...
list.LG <- seq (from=1, to = LGs_I50K_eurekaSSR$n.groups, by=1)
...
link_Group_I50K_eurekaSSR <- lapply (list.LG, function (filt.lg){
  print (str_c ("LG.", filt.lg))
  LGx_I50keurekaSSR <- make_seq (LGs_I50K_eurekaSSR, filt.lg)
...
LG_rcd_eurekaSSR <- lapply (list.LG, function (filt.lg){
  ...
  LGx_rcd <- rcd (input.seq = LGx, rm_unlinked = TRUE)
  
  
})


LG_rcd_I50K_eurekaSSR <- lapply (list.LG, function (filt.lg){
  print (str_c ("LG.", filt.lg))
  LGx <- link_Group_I50K_eurekaSSR [[filt.lg]]
...
LG_rcd_I50K_eurekaSSR_ord  <- lapply (list.LG, function (filt.lg){
  print (str_c ("LG.", filt.lg))
  
  LG.rcd <- LG_rcd_I50K_eurekaSSR  [[filt.lg]]
...
LG_rcd_I50K_eurekaSSR_force  <- lapply (list.LG, function (filt.lg){
  print (str_c ("LG.", filt.lg))
  
  LGx.ord <- LG_rcd_I50K_eurekaSSR_ord [[filt.lg]]
...
LG_I50K_eurekaSSR_dist  <- lapply (list.LG, function (filt.lg){
  
  print (str_c ("LG.", filt.lg))
  
  LGx.dist <- LG_rcd_I50K_eurekaSSR_force [[filt.lg]]
...
write_map (LG_eurekaSSR_dist, "./Data/procdata/map.eurekaSSR.txt")

write_map (LG_I50K_eurekaSSR_dist, "./Data/procdata/map.I50K_eurekaSSR.txt")



## reingreso los dato del mapa para formatear

I50K_eurekaSSR.map <- read_delim (file="./Data/procdata/map.I50K_eurekaSSR.txt", 
                             delim = " ", na = "NA", col_names = FALSE )

I50K_eurekaSSR.map <- I50K_eurekaSSR.map %>%
                      dplyr::rename (Chr = X1 )%>%
...
# ahora genero pmap
write_delim (I50K_eurekaSSR.map , file ="./Data/procdata/pmap_I50K_eurekaSSR.csv",
             delim = ",", na = "-")

### Cargo las matriz geno que salio del bins de onemap modifcada para el formato
I50k_eurekaSSR <- read_delim (file="./Data/procdata/I50K_eurekaSSR.txt" ,
                                delim = " ", na = "-")

I50k_eurekaSSR <- I50k_eurekaSSR %>%
                  dplyr::select (-x2) %>%
                  dplyr::rename (Name=x1)

mrk <- unique (I50K_eurekaSSR.map$Name )


I50k_eurekaSSR <- I50k_eurekaSSR %>%
                  dplyr::filter (Name %in% mrk)

### es la informacion de los estados alelicos
I50k_eurekaSSR.1 <- I50k_eurekaSSR  %>%
                    dplyr::select ( -Name )

IDs.mark <- I50k_eurekaSSR %>%
            dplyr::select ( Name )

# reemplazo los alelos
I50k_eurekaSSR.1 [I50k_eurekaSSR.1 =="a"] <- "A" 
I50k_eurekaSSR.1 [I50k_eurekaSSR.1 =="b"] <- "B" 

#uno las matrices
I50k_eurekaSSR.2 <- bind_cols (IDs.mark, I50k_eurekaSSR.1)


write_delim (I50k_eurekaSSR.2, file="./Data/procdata/geno_I50K_eurekaSSR.csv",
             delim = ",", na = "-")


### generar la pheno
colnames (I50k_eurekaSSR.2)
id <- colnames (I50k_eurekaSSR.2) [2:81]
random <- rnorm(80, 0, 1)
length (random)
length(id)
pheno_I50k_eurekaSSR <- rbind(id, random)
pheno_I50k_eurekaSSR <- as.data.frame (I50k_eurekaSSR)
colnames (I50k_eurekaSSR) <- id
pheno_I50k_eurekaSSR  <- pheno_I50k_eurekaSSR[-1,]

pheno_I50k_eurekaSSR <- pheno_I50k_eurekaSSR %>%
                  dplyr::mutate (X="trait")%>%
                  dplyr::select (X,everything())


write_delim (pheno_I50k_eurekaSSR, file ="./Data/procdata/pheno_I50k_eurekaSSR.csv",
             delim = ",", na = "-")

## Datos

I50k_eurekaSSR.qtl <- read.cross (format="tidy",
                                dir="./Data/procdata",
                                genfile ="geno_I50k_eurekaSSR.csv", 
                                mapfile ="pmap_I50k_eurekaSSR.csv" , 
                                phefile ="pheno_I50k_eurekaSSR.csv" , 
                                na.strings="-",
                                alleles=c("a","b"),
                                estimate.map=FALSE, 
...
linkmap (eurekaSSR.qtl, c(1))

write.cross (eurekaSSR.qtl, format="qtlcart",
            filestem="./Data/procdata/eurekaSSR_qtlCart",  digits=4)





head (eurekaSSR_riself )
